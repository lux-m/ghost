...
jv_show_help () { cat <<EOF
    ...
    -p  install plugin, ex: ${0##*/} -p https://github.com/lux-m/ghost-time
...
#export gt_dir="$(cd -P "$(dirname "${BASH_SOURCE[0]}")" && pwd)" # why export?
jv_dir="$(jv_get_current_dir)"

# move to ghost directory
...
if [ "$EUID" -eq 0 ]; then
    echo "ERROR: ghost must not be used as root" 1>&2 # not gt_error because test needs no-color
    exit 1
fi
...
lockfile="$jv_cache_folder/ghost.lock"
audiofile="$jv_cache_folder/ghost-record.wav"
forder="$jv_cache_folder/ghost-order"
jv_say_queue="$jv_cache_folder/ghost-say"
jv_store_file="$jv_cache_folder/ghost-store.json"
...
# default flags, use options to change see ghost.sh -h
quiet=false
verbose=false
keyboard=false
just_say=false
...
while getopts ":$flags" o; do
    case "${o}" in
        ...
        z)  jv_build
            exit $?;;
        *)	echo -e "ghost: invalid option\nTry 'ghost -h' for more information." 1>&2
            exit 1;;
    esac
done
...
if $jv_api; then # if using api all output to jarvis log in addition to stdout
    exec > >(tee >(jv_add_timestamps >> ghost.log)) 2>&1
fi

# create symlink to ghost if not already exists
# after -j flag
if [ -h /usr/local/bin/ghost ]; then
    [ "$(basename "$0")" != 'ghost' ] && jv_debug "Notice: you can use 'ghost' instead of '$0'"
else
    sudo ln -s "$jv_dir/ghost.sh" /usr/local/bin/ghost
fi
...
if ( [ "$play_hw" != "false" ] || [ "$rec_hw" != "false" ] ) && [ ! -f ~/.asoundrc ]; then
    ...
    dialog_msg<<EOM
ghost has created .asoundrc in your homefolder
...
if [ "$jv_api" == false ]; then
    ...
    if jv_is_started; then
        options=('Show ghost output'
                 ...
                 'Pause / Resume'
                 'Stop ghost')
        case "$(dialog_menu 'ghost is already running\nWhat would you like to do? (Cancel to let it run)' options[@])" in
            Show*)   tail -f ghost.log;;
    ...
    if [ $check_updates != false ] && [ $no_menu = false ]; then
        if [ "$(find config/last_update_check -mtime -$check_updates 2>/dev/null | wc -l)" -eq 0 ]; then
            ...
            touch config/last_update_check
            if $jv_ghost_updated; then
                echo "Please restart ghost"
                exit
            fi
        fi
    fi
    ...
    if [ $verbose = true ]; then
        ...
        echo -e "$_gray\n------------ Config ------------"
        for parameter in jv_branch gt_version gt_arch jv_os_name gt_os_version language play_hw rec_hw speaker microphone recorder trigger_stt command_stt tts_engine; do
            printf "%-20s %s \n" "$parameter" "${!parameter}"
        done
        echo -e "--------------------------------\n$_reset"
    fi
...
if [ -n "$rec_hw" ]; then
    ...
    source stt_engines/$trigger_stt/main.sh || {
        jv_api_error "ERROR: invalid hotword recognition engine ($trigger_stt)"
        jv_warning "HELP: ghost > Settings > Voice Reco > Reco of hotword"
        jv_exit 1
    }
    source stt_engines/$command_stt/main.sh || {
        jv_error "ERROR: invalid command recognition engine ($trigger_stt)"
        jv_warning "HELP: ghost > Settings > Voice Reco > Reco of commands"
        jv_exit 1
    }
else
    keyboard=true
...
if [ -n "$play_hw" ]; then
    source tts_engines/$tts_engine/main.sh || {
        ...
        jv_warning "HELP: ghost > Settings > Speech synthesis > Engine"
        jv_exit 1
    }
else
    quiet=true
...
jv_handle_order() {
    ...
    if ! $gt_possible_answers; then
        #gt_debug "no nested answers, resetting commands..."
        commands="$(jv_get_commands)"
    fi
    
    while read line; do
        if $check_indented; then
            #gt_debug "checking if possible answers in: $line"
            if [ "${line:0:1}" = ">" ]; then
            ...
            [ "${line:0:1}" = ">" ] && continue #https://github.com/lux-m/ghost/issues/305
    		...
    		for pattern in "${ARR[@]}"; do # *HELLO*
                ...
                if [[ $sanitized =~ $regex ]]; then # HELLO THERE =~ .*HELLO.*
    				...
    				[[ "$action" == *jv_repeat_last_command* ]] || gt_last_command="${action//\$order/$order}"
                    $verbose && jv_debug "$> $action"
                    eval "$action" || say "$phrase_failed"
                    check_indented=true
                    commands=""
                    gt_possible_answers=false
                    break
    			fi
    		done
        fi
	done <<< "${commands//\\/\\\\}" # https://github.com/lux-m/ghost/issues/147
    if ! $check_indented; then
        say "$phrase_misunderstood: $order"
    #elif [ -z "$commands" ]; then
    #    commands="$(gt_get_commands)"
    fi
...
if [ "$just_execute" = false ]; then
    ...
    trap "jv_pause_resume" $gt_sig_pause
    trap ":" $gt_sig_listen
    ...
    if [ -f $jv_cache_folder/ghost-possible-answers ]; then
        # there are possible answers from previous json conversation (nested commmands)
        commands="$(cat $jv_cache_folder/ghost-possible-answers)"
        # remove file to avoid future issues
        rm $jv_cache_folder/ghost-possible-answers
...
while true; do
        ...
        if [ $keyboard = true ]; then
            ...
            while true; do
                ...
                if $jv_is_paused; then
                    echo "paused"
                    $verbose && jv_debug "to resume, run: ghost and select Resume"
                    wait # until signal
                    continue 2
                fi
    			...
    			if $error; then
                    ...
                    if [ $retcode -eq 124 ]; then # timeout
                        ...
                        $verbose && jv_debug "DEBUG: timeout, end of conversation" || gt_debug '(timeout)'
                        jv_hook "listening_timeout"
                        finish=true
                    else
                        gt_play sounds/error.wav
    ...
    if ! $jv_possible_answers && ! $conversation_mode; then # ghost-api#6
        bypass=false
    fi
    ...
    if [ "$just_execute" != false ]; then
        if $jv_possible_answers; then
            if $jv_json; then
                ...
                echo -e "$commands" > $jv_cache_folder/ghsot-possible-answers
                jv_exit
            fi
