...
configure () {
    ...
    case "$1" in
        ...
        check_updates)         options=('Always' 'Daily' 'Weekly' 'Never')
                               case "$(dialog_select "Check Updates when ghost starts up\nRecommended: Daily" options[@] "Daily")" in
        ...
        jv_use_bluetooth)      eval "$1=\"$(dialog_yesno "Do you want to use a bluetooth Speaker?\nThis will start PulseAudio" "${!1}")\""
                               $gt_use_bluetooth && pulseaudio --start || pulseaudio --kill
                               ;;
        ...
        load)
            source defaults/ghost-config-default.sh
            #[ -f ghost-config.sh ] && source ghost-config.sh # backward compatibility
            ...
            if [ "$tts_engine" == "voxygen" ]; then
                ...
                jv_debug "See https://github.com/lux/ghost/issues/446"
...
wizard () {
    ...
    [ -f ghost-commands ] || cp defaults/ghost-commands-default ghost-commands
    [ -f ghost-events ] || cp defaults/ghost-events-default ghost-events
    
    dialog_msg "Hello, my name is ghost, nice to meet you"
    configure "language"
