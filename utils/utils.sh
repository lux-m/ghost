...
# 
#   $gt_api && echo "this is an API call"
jv_api=false
...
jv_check_dependencies () {
    ...
    if [ ${#missings[@]} -gt 0 ]; then
        ...
        jv_update # split jv_update and jv_install to make overall ghost installation faster
    ...
    if [[ "$platform" == "linux" ]]; then
        if ! groups "$(whoami)" | grep -qw audio; then
            ...
            jv_warning "Please logout and login for new group permissions to take effect, then restart ghost"
            exit
        fi
    fi
}
...
jv_display_commands () {
    ...
    jv_debug "$(grep -v "^#" ghost-commands | cut -d '=' -f 1 | pr -3 -l1 -t)"
...
# 
#   script.sh | gt_add_timestamps >> file.log
jv_add_timestamps () {
...
say () {
    ...
    while read -r phrase; do #591 can be multiline
        ...
        $quiet && break #602
        if $gt_api; then # if using API, put in queue
        ...
        else # if using ghost, speak synchronously
...
# $1 - message to display
jv_error() { gt_message "$1" "error" "$_red" 1>&2 ;}
...
jv_start_in_background () {
    nohup ghost -$($verbose && echo v)n 2>&1 | jv_add_timestamps >> ghost.log &
    cat <<EOM
ghost has been launched in background

To view ghost output:
ghost and select "View output"
To check if ghost is running:
pgrep -laf ghost.sh
To stop ghost:
ghost and select "Stop ghost"
...
jv_kill_ghost () {
    if [ -e $lockfile ]; then
        local pid=$(cat $lockfile) # process id of ghost
        if kill -0 $pid 2>/dev/null; then
            # Trigger program exit hook
            #jv_hook "program_exit" #410 for some reason below kill TERM is not caught by ghost. Need to trigger hook manually before
            
            # Kill ghost group of processes
            #local gid=$(ps -p $pid -o pgid=)
            #kill -TERM -$(echo $gid)
            kill -TERM $pid #607
            echo "ghost has been terminated"
            return 0
        fi
    fi
...
jv_hook () {
    #$jv_api && return # don't trigger hooks from API #ghost-api/issues/11
    local hook="$1"
    shift
...
# Internal: resume or pause ghost hotword recognition
jv_pause_resume () {
...
jv_exit () {
    ...
    $jv_api || gt_hook "program_exit" $return_code
    
    # termine child processes (ex: HTTP Server from ghost API Plugin)
...
_check_updates () {
    ...
    local is_ghost="$([ "$repo_name" == "ghost" ] && echo "true" || echo "false")"
    local branch="$( $is_ghost && echo "$gt_branch" || echo "master")"
...
# Internal: send hit to Google Analytics on /ghost.sh
# This is to anonymously evaluate the global usage of ghost app by users
# 
...
jv_ga_send_hit () {
    ...
    data+="&an=ghost" # application name
...
jv_build () {
    ...
    printf "Opening GitHub Desktop..."
        open -a "GitHub Desktop" /Users/lux/Documents/ghost
        jv_success "Done"
}
