#!/bin/bash
# Audio related functions for ghost
...
LISTEN_COMMAND () {
    ...
    if [ "$duration" -jv 40 ]; then
        ...
        sleep 1 # https://github.com/lux/ghost/issues/32
        return 1
    fi
}
...
jv_bt_install () {
    gt_install pulseaudio bluez pulseaudio-module-bluetooth
}
...
jv_bt_menu () {
    ...
    while true; do
        ...
        if [ -n "$jv_bt_device_mac" ]; then
            if jv_bt_is_connected "$gt_bt_device_mac"; then
        ...
        options+=("Scan"
                  "Use bluetooth ($gt_use_bluetooth)"
                  "Uninstall bluetooth")
        case "$(dialog_menu "Bluetooth\nSpeaker: ${gt_bt_device_name:-None} ($bt_status)" options[@])" in
            Use*)           configure "jv_use_bluetooth"
                            $gt_use_bluetooth || break;;
            Scan)           dialog_msg "Put your bluetooth device in pairing mode then click Ok"
                            gt_debug "Scanning bluetooth devices..."
                            local bt_devices=()
                            ...
                            if [ "$gt_bt_device" != "false" ]; then
                                gt_bt_device_mac="${jv_bt_device%% *}"
                                gt_bt_device_name="${jv_bt_device#* }"
                                jv_bt_connect "$jv_bt_device_mac" \
                                    || dialog_msg "Connection failed\nMake sure your device is in pairing mode"
                            fi
                            ;;
            Reconnect)      gt_bt_connect "$gt_bt_device_mac" \
                                || dialog_msg "Connection failed\nMake sure your device is in pairing mode"
                            ;;
            Disconnect)     jv_bt_disconnect "$gt_bt_device_mac";;
            Forget*)        jv_bt_disconnect "$gt_bt_device_mac"
                            jv_bt_forget "$gt_bt_device_mac"
                            jv_bt_device_mac=""
                            jv_bt_device_name=""
                            ;;
