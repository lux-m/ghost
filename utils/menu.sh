...
jv_menu_main () {
    while true; do
        options=('Start ghost'
                 'Settings'
                 'Commands (what ghost can understand and execute)'
                 'Events (what ghost monitors and notifies you about)'
                 'Plugins (commands from community)'
                 'Search for updates'
                 'Help / Report a problem'
                 'About')
        case "`dialog_menu \" Jarvis - v$gt_version\n$headline\" options[@]`" in
            Start*)
                while true; do
                    options=('Start normally' 'Troubleshooting mode' 'Keyboard mode' 'Start as a service')
                    case "`dialog_menu 'Start ghost' options[@]`" in
                        "Start normally")
                            break 2;;
...
EOM
                ...
                configure "save";;
            Commands*)
                editor ghost-commands
                #update_commands
                ;;
            Events*)
                dialog_msg <<EOM
    WARNING: ghost currently uses Crontab to schedule monitoring & notifications
    ...
    If you already have crontab rules defined, add them to JARVIS events:
        crontab -l >> ghost-events
    Press [Ok] to start editing Event Rules
EOM
                editor ghost-events &&
                crontab ghost-events -i;;
            Plugins*)
                jv_menu_store
                ;;
            Help*)
    ...
    https://github.com/lux/ghost/issues/new

    Just want to discuss?
    http://openghost.com/content/disqus
EOM
                ;;
    ...
    You like ghost? consider making a 1€ donation:
    http://openghost.com/content/

    ghost is freely distributable under the terms of the MIT license.
EOM
                ;;
            "Search for updates")
                jv_check_updates
...
menu_store_browse () { # $1 (optional) sorting, $2 (optionnal) space separated search terms    
    while true; do        
        ...
        local plugin_url=$(store_get_field "$plugin_title" 'repo') #https://github.com/lux/ghost
...
jv_menu_store () {
    ...
    while true; do
        ...
        options=("Installed (${#nb_installed[@]})"
                 ...
                 "Install from URL" #TODO also as ghost option argument
                 "Create a Plugin")
            ...
            Installed*) if [ "${#nb_installed[@]}" -gt 0 ]; then
                            ...
                            while true; do
                                ...
                                for plugin in $(ls plugins_installed); do
                                    options+=("$plugin$(gt_plugin_is_enabled "$plugin" || echo " (disabled)")")
                                done
