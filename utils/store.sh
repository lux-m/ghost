...
store_init () {
    ...
    #export store_json="$(curl -s http://openghost.com/all.json)" # why export?
    #export store_json_lower="$(echo "$store_json" | tr '[:upper:]' '[:lower:]')"
}

jv_store_update () {
    printf "Retrieving plugins database..."
    curl -s "https://www.openghost.com/all.json" -H 'User-Agent: Mozilla/5; Windows NT 5.1; en-US; rv:1.8.1.13) Gecko/20080311 Firefox/2.0.0.13' > "$gt_store_file" #617
    jv "Done"
}
...
jv_store_list_plugins_recommended () {
    cat "$gt_store_file" | jq --raw-output ".nodes | map(select(.node.recommended==\"Recommended\")) | .[].node.title"
}
...
jv_store_search_plugins () { # $1:space separated search terms
    ...
    cat "$gt_store_file" | jq -r ".nodes[] | select(.node.tags | contains(\"$term\")) | .node.title" #TODO create new keyword field on plugins?
}

store_get_field () { # $1:plugin_name, $2:field_name
    cat "$gt_store_file" | jq -r ".nodes | map(select(.node.title==\"$1\")) | .[0].node.$2"
}

store_get_field_by_repo () {
    cat "$gt_store_file" | jq -r ".nodes | map(select(.node.repo==\"$1\")) | .[0].node.$2"
}
...
store_install_plugin () { # $1:plugin_url
    ...
    git clone "$1.git" #https://github.com/lux/ghost.git
    if [[ $? -eq 0 ]]; then
        cd "$plugin_name"
        source install.sh
        if [[ -s "config.sh" ]]; then
...
store_plugin_uninstall () { # $1:plugin_name
    ...
    source "plugins_installed/$1/uninstall.sh" # access to ghost variables
    rm -rf "plugins_installed/$1"
    #jv_plugins_order_rebuild # in gt_plugin_disable
    #cd plugins_installed/
}
...
jv_store_create_plugin () {
    ...
    [ -z "$username" ] && return
    local reponame="ghost-"
    while true; do
        reponame="$(dialog_input "Your plugin name (short and self-explanatory)\nex: ghost-cookbook" "$reponame")"
        reponame="$(jv_sanitize "$reponame" "-")" # sanitize special chars
        [[ "$reponame" == ghost-* ]] || reponame="ghost-$reponame" # make sure plugin name starts with ghost-
    ...
    git clone "https://github.com/lux/ghost-plugin" .
...
To help you develop, commit and register your plugin on ghost store, visit:
http://openghost.com/content/publish-your-plugin
EOM
}
